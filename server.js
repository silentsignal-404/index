...
function insertDefaultData() {
  ...
  db.run(`INSERT OR IGNORE INTO achievements (name, rank, year, insight, signal_strength, is_active) VALUES 
    ...
    'TexSaw CTF 2026',
    '80th',
    'Currently Active - 80th/510 teams',
  ...
  const philosophyQuotes = [
    'We don\'t chase noise. We analyze signals.',
  ...
  // Default skills
  const defaultSkills = [
    { name: 'Web Exploitation', description: 'Offensive Depth', percentage: 95 },
    { name: 'OSINT', description: 'Intelligence Gathering', percentage: 92 },
    { name: 'Binary Exploitation', description: 'System Control', percentage: 90 },
    { name: 'Reverse Engineering', description: 'Code Analysis', percentage: 85 },
    { name: 'Cryptography', description: 'Cipher Breaking', percentage: 80 },
    { name: 'Digital Forensics', description: 'Evidence Recovery', percentage: 78 }
  ];

  defaultSkills.forEach(skill => {
    db.run(`INSERT OR IGNORE INTO skills (name, description, percentage) VALUES (?, ?, ?)`, 
      [skill.name, skill.description, skill.percentage]);
  });
}
...
app.get('/api/public/skills', (req, res) => {
  db.all('SELECT * FROM skills ORDER BY percentage DESC', (err, rows) => {
    if (err) {
      return res.status(500).json({ error: 'Database error' });
    }
    res.json(rows);
...
app.put('/api/admin/members/:id', authenticateToken, (req, res) => {
  ...
  db.run(`UPDATE members SET name = ?, alias = ?, role = ?, description = ?, 
          skills = ?, avatar_letter = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
...
app.post('/api/admin/skills', authenticateToken, (req, res) => {
  ...
  db.run(`INSERT INTO skills (name, description, percentage, updated_at) 
          VALUES (?, ?, ?, CURRENT_TIMESTAMP)`,
...
app.put('/api/admin/skills/:id', authenticateToken, (req, res) => {
  ...
  db.run(`UPDATE skills SET name = ?, description = ?, percentage = ?, 
          updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
    [name, description || '', percentage, id],
    function(err) {
      if (err) {
        return res.status(500).json({ error: 'Database error' });
      }
...
app.put('/api/admin/messages/:id/read', authenticateToken, (req, res) => {
  ...
  db.run('UPDATE contact_messages SET is_read = 1 WHERE id = ?', [id], function(err) {
    if (err) {
      return res.status(500).json({ error: 'Database error' });
    }
    if (this.changes === 0) {
